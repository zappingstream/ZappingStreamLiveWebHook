...
async Task ProcesarVideoAsync(string videoId, string channelId)
{
    try
    {
        ...
        if (estaEnVivo)
        {
            ...
            app.Logger.LogInformation("Canal {ChannelName} empezó a transmitir en vivo vía Webhook.", channelName);
        ...
        else
        {
            ...
            if (estabaEnVivo && videoVivoActualId != videoId && !string.IsNullOrEmpty(videoVivoActualId))
            {
                ...
                app.Logger.LogInformation("Canal {ChannelName} actualizado vía Webhook.", channelName);
